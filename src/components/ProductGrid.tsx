  ...
  return (
    <section id="collection" className="container py-20 md:py-28">
      <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
        <div className="text-gold text-xs tracking-luxe uppercase">{productGridSection.eyebrow || "Best-sellers"}</div>
        <h2 className="font-serif text-4xl md:text-5xl text-bordeaux">
          {productGridSection.heading || "Les pièces que"} <em>{productGridSection.headingEm || "vos clients adorent"}</em>
        </h2>
        <p className="text-bordeaux/60">
          {productGridSection.description || ""}
        </p>
      </div>
      ...
      <div className="text-center mt-16">
        ...
        >
          {productGridSection.ctaText || "Voir tout le catalogue →"}
        </Link>
      </div>
    </section>
  );
